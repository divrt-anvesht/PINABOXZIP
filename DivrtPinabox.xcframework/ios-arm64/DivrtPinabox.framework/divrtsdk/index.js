...
let getBeaconListResponse, beaconData, nfcData, qrCode, autoSelectMode, garageLocation = {}, loopData = {},
    ...
    firstBleHitTime,techType = 'BLE',bookingID,uid,gateDatainitialised = false;
        ...
        fetch(url, {
            ...
            response.json().then(obj => {
                try {
                    ...
                    J2N_printLog("openGatePedestrian failure" + e.message)
                    J2N_postOnFailure("Gate open pedestrian failure");
                    pushException("openGatePedestrian response inner" + e.message)
                    pushLogs();
                }
            ...
            pushException("openGate response outer" + e.message)
        });
    } catch (e) {
        showProgress(false);
        debugLog("openGatePedExcep " + e.message)
        J2N_postOnFailure("Failed to open gate. Please try again");
        pushException("openGate " + e.message)
    }
}
...
function prefetchHardwareConfiguration(pinaConfigJson) {
    validatePinaConfig(pinaConfigJson)
    getHardwareConfiguration(function () {
        J2N_printLog(`~completedHardWareConfigruation`);
    });
...
function displayWebViewAndStartSDK(pinaConfigJson, hardwareConfigJson) {
    try {
//        alert("This is prebundle JS")
        debugLog("This is prebundle JS")
        if (pinaConfigJson != undefined) {
            let newPinaConfig = JSON.parse(pinaConfigJson)
            if ((newPinaConfig.pinaConfigParams.zid != pinaConfig.pinaConfigParams.zid) || (newPinaConfig.pinaConfigParams.gateType != getBeaconListResponse.data.type)) {
...
function getHardwareConfiguration(callback) {
    try {
        ...
        fetch(url, {
            ...
            .then(response => {
                ...
                if (status == true) {
                    getBeaconListResponse = response;
                    J2N_forceJSupdate(JSON.stringify(response))
                    initHardwareConfig();
                    callback();
                } else {
//                    alert(message)
            ...
            J2N_printLog(e.message)
            showProgress(false);
            pushException("getHardwareConfiguration response" + e.message)
        });
    } catch (e) {
        showProgress(false);
        pushException("getHardwareConfiguration " + e.message)
    }
}
        ...
        fetch(url, {
            ...
            response.json().then(obj => {
                try {
                    ...
                    pushException("openGate response inner" + e.message)
                    pushLogs();
                }
